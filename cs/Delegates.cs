...
namespace cs {
    public class Delegates {
        ...
        public static int sum2(int N, Func<int, int> f) {   // Same as above, just different notation, takes int, returns int
            int sum = 0;
            for (int i = 0; i <= N; i++) {
                sum += f(i);
            }
        ...
        static void DelegateExample() {
            ...
            // StringOp implies that the x param will be string
            //StringOp toApply = (x) => { a.Add(x); };

            // add to b if x begins with vowel
            //toApply += (x) => {
            //    if ("AEIOU".Contains(x[0]))
            //        b.Add(x);
            //};

            // add to c at position length of word
            //toApply += (x) => {
       ...
       //     return;
        ...
        static void MulticastDelegateExample() {
            ...
            func += f2;
            func += f3;
            func(4); // prints 4, then 8, then 12
        }
